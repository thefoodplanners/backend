...
import co.uk.foodgen.services.models.MetricViews.*
...
object ProgressChartService:
  ...
  def fetchMetrics(
    ...
    for
      mealViews <- MealRepository.selectMealsByPeriod(date, period, userId)
      fullMetricView =
        period match
          case Period.Day =>
